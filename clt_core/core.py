...
import clt_core.util.math as clt_math
from clt_core.util.viz import plot_box, plot_frame
import matplotlib.pyplot as plt
from clt_core.util.info import Logger
import clt_core.util.info as clt_info
import pickle
import os
...
class Object:
    def __init__(self, name, pos=np.zeros(3), quat=Quaternion(),
                 size=[0.01, .01, .01], body_id=None, class_=None, color=(0, 0, 1)):
        """
...
def train(env, agent, mdp, logger, n_episodes=10000, episode_max_steps=50, save_every=500, exp_name='', seed=0):
    train_data = []
...
def eval(env, agent, mdp, logger, n_episodes=10000, episode_max_steps=50, exp_name='', seed=0):
    ...
    rng = np.random.RandomState()
    rng.seed(seed)
    if agent.loaded is not None:
        logger.set_key('agent_loaded', agent.loaded)
    for i in range(n_episodes):
